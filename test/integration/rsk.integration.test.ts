...
import { type Config, ConfigLive } from "../../src/config.js";
import { buildTopicFilter, fetchLogs } from "../../src/pipeline/LogFetcher.js";
import {
	EventDecoder,
	EventDecoderLive,
} from "../../src/services/EventDecoder.js";
import {
	RpcProvider,
	RpcProviderLive,
} from "../../src/services/RpcProvider.js";
import { GOVERNOR_ABI, STRIF_TOKEN_ABI } from "../fixtures/abi.js";
...
const runIntegration = process.env.RUN_RSK_INTEGRATION === "true";
...
> =>
	Effect.gen(function* () {
		...
		for (let index = 0; index < scenario.maxWindows; index += 1) {
			...
			const logsCount = chunks.flatMap((chunk) => chunk).length;
			if (logsCount > 0) {
		...
		it(`fetches and decodes logs for ${scenario.name}`, async () => {
			...
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					...
					const logs = chunks.flatMap((chunk) => chunk);
